...
export class App {
  ...
  configureRouter(config: RouterConfiguration, router: Router) {
    config.map([
      { route: ['', 'test-route-1'], moduleId: PLATFORM.moduleName('./routes/test-route-1'), nav: false, title: 'TestRoute1', activationStrategy: activationStrategy.replace },
      { route: 'test-route-2', moduleId: PLATFORM.moduleName('./routes/test-route-2'), nav: false, title: 'TestRoute2', activationStrategy: activationStrategy.replace },
    ]);
    this.router = router;
  }
